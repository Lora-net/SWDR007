...
void *wifi_scan_done_context = &application_context;
    ...
    if (xQueueReceive(application_context.event_queue, &event, portMAX_DELAY) == pdTRUE) {
      ...
      switch (event) {
        ...
        case EVENT_TYPE_CONNECTION_REQUEST:
         ...
         case EVENT_TYPE_WIFI_SCAN_START:
           app_log_info("app: start wifi scan evt");
           start_wifi_scan( &application_context );
           break;
...
static void timer_scan_callback( void* parameter )
{
  ...
  if(cnt % WIFI_SCAN_EVERY_NTH_TIME == (WIFI_SCAN_EVERY_NTH_TIME-1)) {
      queue_event( g_event_queue, EVENT_TYPE_WIFI_SCAN_START );
  } else {
