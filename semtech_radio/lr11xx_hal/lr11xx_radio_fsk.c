...
#define LR11XX_FSK_PHY_HEADER_LENGTH 2
#define LR11XX_FSK_WHITENING_SEED 0x01FF
#define LR11XX_FSK_MAX_PAYLOAD_LENGTH 255
#define LR11XX_FSK_SYNC_WORD_LENGTH_IN_RX 3
...
static void radio_mp_to_lr11xx_mp( lr11xx_radio_mod_params_gfsk_t*              fsk_mp,
                                   const sid_pal_radio_fsk_modulation_params_t* mod_params );

static void radio_pp_to_lr11xx_pp( lr11xx_radio_pkt_params_gfsk_t*          fsk_pp,
...
int32_t radio_fsk_process_sync_word_detected( halo_drv_semtech_ctx_t* drv_ctx )
{
    ...
    do
    {
        ...
        /* Below code is taken from Semtech Halo driver. It's purpose to handle non standrat GFSK
           ...
           what they are */
        while( rx_buffer_ptr_position <= LR11XX_FSK_PHY_HEADER_LENGTH + 2 )
        {
            lr11xx_regmem_read_regmem32( drv_ctx, 0x00F20384, &rx_buffer_ptr_position, 1 );
            rx_buffer_ptr_position &= 0x0FFF0000;
            rx_buffer_ptr_position = rx_buffer_ptr_position >> 16;
        }

        lr11xx_regmem_read_buffer8( drv_ctx, buffer, 0x00, LR11XX_FSK_PHY_HEADER_LENGTH + 2 );

        rx_buffer_ptr_position = buffer[1] + LR11XX_FSK_PHY_HEADER_LENGTH;
        lr11xx_regmem_write_regmem32_mask( drv_ctx, 0x00F20368, 0xFFF00000, rx_buffer_ptr_position << 20 );
...
int32_t radio_fsk_process_rx_done( halo_drv_semtech_ctx_t* drv_ctx, radio_fsk_rx_done_status_t* rx_done_status )
{
    ...
    do
    {
        if( ( buffer[RADIO_FSK_SYNC_WORD_VALID_MARKER_OFFSET] << 8 |
              buffer[RADIO_FSK_SYNC_WORD_VALID_MARKER_OFFSET + 1] ) != RADIO_FSK_SYNC_WORD_VALID_MARKER )
        {
        ...
        // LR11XX_FSK_MAX_PAYLOAD_LENGTH
        if( lr11xx_regmem_read_buffer8( drv_ctx, buffer,
                                        rx_buffer_status.buffer_start_pointer + LR11XX_FSK_PHY_HEADER_LENGTH,
                                        length_temp ) != LR11XX_STATUS_OK )
        {
            err             = RADIO_ERROR_IO_ERROR;
        ...
        if( lr11xx_radio_get_gfsk_pkt_status( drv_ctx, &pkt_status ) != LR11XX_STATUS_OK )
        {
            err             = RADIO_ERROR_IO_ERROR;
            *rx_done_status = RADIO_FSK_RX_DONE_STATUS_TIMEOUT;
            break;
                                                   ...
                                                   sid_pal_radio_data_rate_t              data_rate )
{
    ...
    case SID_PAL_RADIO_DATA_RATE_50KBPS:
        ...
        mod_params->bandwidth   = ( uint8_t ) LR11XX_RADIO_GFSK_BW_156200;
        mod_params->mod_shaping = ( uint8_t ) LR11XX_RADIO_GFSK_PULSE_SHAPE_BT_1;
        break;
    case SID_PAL_RADIO_DATA_RATE_150KBPS:
        mod_params->bit_rate    = RADIO_FSK_BR_150KBPS;
        mod_params->freq_dev    = RADIO_FSK_FDEV_37_5KHZ;
        mod_params->bandwidth   = ( uint8_t ) LR11XX_RADIO_GFSK_BW_312000;
        mod_params->mod_shaping = ( uint8_t ) LR11XX_RADIO_GFSK_PULSE_SHAPE_BT_05;
        break;
    case SID_PAL_RADIO_DATA_RATE_250KBPS:
        mod_params->bit_rate    = RADIO_FSK_BR_250KBPS;
        mod_params->freq_dev    = RADIO_FSK_FDEV_62_5KHZ;
        mod_params->bandwidth   = ( uint8_t ) LR11XX_RADIO_GFSK_BW_467000;
        mod_params->mod_shaping = ( uint8_t ) LR11XX_RADIO_GFSK_PULSE_SHAPE_BT_05;
        break;
    default:
        return RADIO_ERROR_INVALID_PARAMS;
...
int32_t sid_pal_radio_prepare_fsk_for_rx( sid_pal_radio_fsk_pkt_cfg_t* rx_pkt_cfg )
{
    ...
    do
    {
        ...
        if( rx_pkt_cfg->phy_hdr == NULL || rx_pkt_cfg->packet_params == NULL || rx_pkt_cfg->sync_word == NULL )
        {
            break;
        }
        ...
        f_pp->payload_length       = LR11XX_FSK_MAX_PAYLOAD_LENGTH;
...
int32_t sid_pal_radio_prepare_fsk_for_tx( sid_pal_radio_fsk_pkt_cfg_t* tx_pkt_cfg )
{
    ...
    do
    {
        ...
        if( tx_pkt_cfg->phy_hdr == NULL || tx_pkt_cfg->packet_params == NULL || tx_pkt_cfg->sync_word == NULL ||
            tx_pkt_cfg->payload == NULL )
        {
            break;
        }
...
{
    UNUSED( sync_word_length );
    if( lr11xx_radio_set_gfsk_sync_word( lr11xx_get_drv_ctx( ), sync_word ) != LR11XX_STATUS_OK )
    {
...
int32_t sid_pal_radio_set_fsk_modulation_params( const sid_pal_radio_fsk_modulation_params_t* mod_params )
{
    ...
    assert( drv );
    if( lr11xx_radio_set_gfsk_mod_params( drv, &fsk_mp ) != LR11XX_STATUS_OK )
    {
        return RADIO_ERROR_HARDWARE_ERROR;
    }
...
int32_t sid_pal_radio_set_fsk_packet_params( const sid_pal_radio_fsk_packet_params_t* packet_params )
{
    ...
    halo_drv_semtech_ctx_t* drv = lr11xx_get_drv_ctx( );
    assert( drv );
    if( lr11xx_radio_set_gfsk_pkt_params( drv, &fsk_pp ) != LR11XX_STATUS_OK )
    {
        return RADIO_ERROR_HARDWARE_ERROR;
    }
...
{
    uint32_t num_symb = LR11XX_US_TO_SYMBOLS( delay_micro_secs, mod_params->bit_rate );
    return num_symb;
}
                                   ...
                                   const sid_pal_radio_fsk_modulation_params_t* mod_params )
{
    ...
    _Static_assert( LR11XX_RADIO_GFSK_BW_93800 == SID_PAL_RADIO_FSK_BW_93800, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_BW_117300 == SID_PAL_RADIO_FSK_BW_117300, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_BW_156200 == SID_PAL_RADIO_FSK_BW_156200, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_BW_312000 == SID_PAL_RADIO_FSK_BW_312000, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_BW_467000 == SID_PAL_RADIO_FSK_BW_467000, "Invalid convertion" );
    fsk_mp->bw_dsb_param = ( lr11xx_radio_gfsk_bw_t ) mod_params->bandwidth;

    _Static_assert( SID_PAL_RADIO_FSK_MOD_SHAPING_OFF == LR11XX_RADIO_GFSK_PULSE_SHAPE_OFF, "Invalid convertion" );
    _Static_assert( SID_PAL_RADIO_FSK_MOD_SHAPING_G_BT_03 == LR11XX_RADIO_GFSK_PULSE_SHAPE_BT_03,
                    "Invalid convertion" );
    _Static_assert( SID_PAL_RADIO_FSK_MOD_SHAPING_G_BT_05 == LR11XX_RADIO_GFSK_PULSE_SHAPE_BT_05,
                    "Invalid convertion" );
    _Static_assert( SID_PAL_RADIO_FSK_MOD_SHAPING_G_BT_07 == LR11XX_RADIO_GFSK_PULSE_SHAPE_BT_07,
                    "Invalid convertion" );
    _Static_assert( SID_PAL_RADIO_FSK_MOD_SHAPING_G_BT_1 == LR11XX_RADIO_GFSK_PULSE_SHAPE_BT_1, "Invalid convertion" );
                                   ...
                                   const sid_pal_radio_fsk_packet_params_t* packet_params )
{
    ...
    _Static_assert( LR11XX_RADIO_GFSK_PREAMBLE_DETECTOR_OFF == SID_PAL_RADIO_FSK_PREAMBLE_DETECTOR_OFF,
                    "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_PREAMBLE_DETECTOR_MIN_8BITS == SID_PAL_RADIO_FSK_PREAMBLE_DETECTOR_08_BITS,
                    "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_PREAMBLE_DETECTOR_MIN_16BITS == SID_PAL_RADIO_FSK_PREAMBLE_DETECTOR_16_BITS,
                    "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_PREAMBLE_DETECTOR_MIN_24BITS == SID_PAL_RADIO_FSK_PREAMBLE_DETECTOR_24_BITS,
                    "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_PREAMBLE_DETECTOR_MIN_32BITS == SID_PAL_RADIO_FSK_PREAMBLE_DETECTOR_32_BITS,
                    "Invalid convertion" );
    ...
    _Static_assert( LR11XX_RADIO_GFSK_ADDRESS_FILTERING_DISABLE == SID_PAL_RADIO_FSK_ADDRESSCOMP_FILT_OFF,
                    "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_ADDRESS_FILTERING_NODE_ADDRESS == SID_PAL_RADIO_FSK_ADDRESSCOMP_FILT_NODE,
                    "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_ADDRESS_FILTERING_NODE_AND_BROADCAST_ADDRESSES ==
    ...
    _Static_assert( LR11XX_RADIO_GFSK_PKT_FIX_LEN == SID_PAL_RADIO_FSK_RADIO_PACKET_FIXED_LENGTH,
                    "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_PKT_VAR_LEN == SID_PAL_RADIO_FSK_RADIO_PACKET_VARIABLE_LENGTH,
                    "Invalid convertion" );
    ...
    _Static_assert( LR11XX_RADIO_GFSK_CRC_OFF == SID_PAL_RADIO_FSK_CRC_OFF, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_CRC_1_BYTE == SID_PAL_RADIO_FSK_CRC_1_BYTES, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_CRC_2_BYTES == SID_PAL_RADIO_FSK_CRC_2_BYTES, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_CRC_1_BYTE_INV == SID_PAL_RADIO_FSK_CRC_1_BYTES_INV, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_CRC_2_BYTES_INV == SID_PAL_RADIO_FSK_CRC_2_BYTES_INV, "Invalid convertion" );
    ...
    _Static_assert( LR11XX_RADIO_GFSK_DC_FREE_OFF == SID_PAL_RADIO_FSK_DC_FREE_OFF, "Invalid convertion" );
    _Static_assert( LR11XX_RADIO_GFSK_DC_FREE_WHITENING == SID_PAL_RADIO_FSK_DC_FREEWHITENING, "Invalid convertion" );
